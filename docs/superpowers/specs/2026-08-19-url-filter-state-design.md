...
**Timing verified during grilling:** `handleSelectSuggestion` (`Players.tsx:134-141`) resets filters, then uses `requestAnimationFrame` to scroll to and highlight the selected row, relying on the reset having already re-rendered the (now-unfiltered) table by the time the callback fires. Moving the setter from plain `useState` to `useSearchParams`-backed state raised a real question: does `setSearchParams` resolve synchronously enough to preserve that ordering, or could it introduce an async gap? Checked directly against the installed `react-router-dom@7.18.2` source (`node_modules/react-router/dist/development/chunk-62JRHF6Z.mjs:10850-10860`): `setSearchParams` calls `navigate()` synchronously within the same call stack. For this app's plain `<BrowserRouter>` (no data-router loaders), that resolves through React's ordinary render/commit cycle, which always completes before the browser's next paint — and `requestAnimationFrame` only fires after paint. The ordering guarantee holds; no regression here.

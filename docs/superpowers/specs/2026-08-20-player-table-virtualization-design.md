...
- **Known, accepted limitation — not fixed in this plan:** today, every row has `tabIndex={0}` when `onOpenProfile` is set, so a keyboard user can Tab through all rows in sequence. Under virtualization, only mounted rows have a `tabIndex` — tabbing past the last currently-rendered row skips straight past the table rather than scrolling further and continuing. Building real roving-tabindex + keyboard-driven scroll is a substantially larger undertaking than this fix, and the original audit finding was specifically about DOM/render performance, not accessibility. Confirmed during grilling as a deliberate scope decision, not an oversight — worth its own follow-up item if it matters later.
...
- **Scroll-container ownership (resolved during grilling — this determines `PlayerTable`'s prop contract, so it's settled here, not deferred to the plan).** `Players.tsx` currently owns the `<div data-testid="table-wrap" className="overflow-auto" style={{ height: ... }}>` wrapper around `<PlayerTable>`. That wrapper's height is computed from `--toolbar-height` (measured by `Players.tsx`'s own effect) and `--header-height` (set by `Header.tsx`) — page-chrome state a reusable `PlayerTable` component has no business knowing about. So `Players.tsx` keeps owning and rendering that wrapper div, and passes its ref down as a new required prop: `scrollContainerRef: React.RefObject<HTMLDivElement>`. `PlayerTable` uses it as `getScrollElement: () => scrollContainerRef.current`.
...
- **React `key` correctness (verified against the installed type definitions during grilling, not assumed):** each `VirtualItem`'s own `.key` field defaults to `defaultKeyExtractor(index)` — the row's *position* in the current scroll window, not its data identity. `<TableRow>` must keep using `key={rows[virtualItem.index].player_id}` (as today), never `key={virtualItem.key}` — using the position-based key would let React reuse the same DOM node across different players occupying the same scroll position after a re-sort, which is exactly the kind of subtle bug that wouldn't show up in a quick visual check. `virtualItem.index` is confirmed to index into the full `rows` array (not a rendered-subset-relative index), so `rows[virtualItem.index]` is the correct row lookup.
